    ...
    for i = 1, #allPlayers do
        ...
        for _, multipoint in ipairs(XCBuildMultipoints(hitPart)) do
            ...
            if XCConfig.rageTargetMode == "Health" then
                score = getXCHealth(char, plr, hum) or math.huge
...
function getOrCreateScreenEsp(plr)
    ...
    weaponImage.Position = UDim2.fromOffset(10, 3)
    ...
    weaponLabel.Size = UDim2.new(1, -78, 1, -2)
...
function xcNormalizeWeaponIcon(value)
    if type(value) == "number" and value > 0 then
        return "rbxassetid://" .. math.floor(value)
    end
    if type(value) ~= "string" then return nil end
    value = value:match("^%s*(.-)%s*$")
    if value:match("^%d+$") then return "rbxassetid://" .. value end
    if value:match("^rbxassetid://%d+$") or value:match("^rbxasset://") then return value end
    return nil
end
...
function findXCWeaponIcon(weaponName, tool)
    ...
    if not entry or os.clock() >= entry.Expires then
        ...
        if asset then
            ...
            if not icon then
                for index, child in ipairs(asset:GetDescendants()) do
                    if index > 128 then break end
                    local kind = child.Name:lower()
...
function buildXCWeaponViewport(esp, weaponName, tool, character)
    ...
    -- Tiny neon model previews are hard to distinguish. Only render a model
    -- when the player explicitly chooses the 3D style.
    if XCConfig.weaponEspStyle ~= "3D" then return false end
    ...
    if not esp.WeaponViewport then
        ...
        viewport.Size = UDim2.fromOffset(56, 24)
        viewport.Position = UDim2.fromOffset(10, 3)
    ...
    for _, object in ipairs(cloneObjects) do
        ...
        elseif object:IsA("BasePart") then
            ...
            if lower:find("arm", 1, true) or lower:find("hand", 1, true)
                ...
                -- Preserve the original materials and textures so similarly
                -- sized guns still have their own recognizable details.
...
function renderTacticalOverlay()
    ...
    for plr in pairs(activeEspHolders) do
        ...
        if esp.Character ~= char or not isAlive then
            esp.SmoothRect = nil
            esp.Character = char
        end
